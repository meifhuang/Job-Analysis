...
try:
    current = pd.read_csv('software_engineer_jobs_USA.csv',sep=';')
    csv_count = len(current.index)
    num_start = int((csv_count - 2500)/ 15  * 10) 
except:
    num_start = 0
...
while len(jobs_list) < 90:
    ...
    for job in jobs:
        ...
        for meta in meta_data:
            if '$' in meta.text or ',' in meta.text or 'year' in meta.text:
...
df.to_csv("software_engineer_jobs_USA.csv",index=False,mode='a',sep=';',header=None,columns=['job_title','company_name','company_location','salary','skills'])
print('COMPLETED!')
driver.quit()